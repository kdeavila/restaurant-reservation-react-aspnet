...
import type { ApiResponse, TableDetailed, CreateTableDto, Pagination } from "@/types";

export const tablesApi = {
  list: async (params: Record<string, unknown>) => {
    const { data } = await http.get<ApiResponse<TableDetailed[] & { pagination: Pagination }>>("/tables", { params });
    return data;
  },
  available: async (params: { date: string; startTime: string; endTime: string; numberOfGuests: number }) => {
