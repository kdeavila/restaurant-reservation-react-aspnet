...
export const useAvailableTables = (params: { date: string; startTime: string; endTime: string; numberOfGuests: number }) =>
  useQuery({ queryKey: qk.available(params), queryFn: () => tablesApi.available(params), enabled: !!params });
...
export const useUpdateTable = () => {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: ({ id, dto }: { id: number; dto: Record<string, unknown> }) => tablesApi.update(id, dto),
    onSuccess: (_, { id }) => {
      qc.invalidateQueries({ queryKey: qk.tables() });
