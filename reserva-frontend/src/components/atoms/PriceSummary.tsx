  ...
  return (
    <dl className={cn("space-y-2.5 text-sm", className)}>
      ...
      <div className="flex justify-between">
        <dt className="text-(--color-muted-fg)">Precio base</dt>
      ...
      {surcharge !== null && surcharge > 0 && (
        <div className="flex justify-between">
          <dt className="text-(--color-muted-fg)">Recargos aplicados</dt>
          <dd className="tabular-nums font-medium text-(--color-primary)">
            + {formatCurrency(surcharge)}
          </dd>
        </div>
      )}

      <div className="border-t border-(--color-border) pt-2.5 flex justify-between items-baseline">
      ...
      {pendingNote && (
        <p className="text-xs text-(--color-muted-fg) pt-1">
