      ...
      <section className="filter-bar px-8 py-4">
        ...
        >
          <div className="flex flex-wrap items-end gap-3">
            <FormField label="Fecha" icon={<CalendarDays className="size-3.5" />} required>
              <Input type="date" min={todayISO()} className="w-40" {...register("date")} />
            </FormField>

            <FormField label="Inicio" icon={<Clock className="size-3.5" />} required>
            ...
            <FormField label="Comensales" icon={<Users className="size-3.5" />} required>
        ...
        {!hasSearched ? (
          ...
          <>
            <div className="mb-6 flex items-center gap-2 text-sm text-muted-fg">
              ...
              <span>·</span>
              <span>
                duración {durationHours(searchParams.startTime, searchParams.endTime).toFixed(1)}h
              </span>
              <span>·</span>
            ...
            {isLoading ? (
              <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
                {Array.from({ length: 6 }, (_, i) => `availability-skeleton-${i}`).map((key) => (
                  <Skeleton key={key} className="surface-card h-48" />
                ))}
              </div>
            ) : tables.length === 0 ? (
              <EmptyState
