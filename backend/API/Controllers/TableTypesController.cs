...
public class TableTypesController(ITableTypeService tableTypeService) : ControllerBase
{
    ...
    )
    {
        ...
        return Ok(
            ApiResponse<IEnumerable<TableTypeDto>>.SuccessResponse(
                data,
                "Table types retrieved successfully",
    ...
    [HttpGet("{id:int}")]
    [ResponseCache(Duration = 3600, Location = ResponseCacheLocation.Any, NoStore = false)]
    public async Task<ActionResult<ApiResponse<TableTypeDto>>> GetById(
        int id,
        CancellationToken ct = default
    )
    {
        ...
        if (result.IsFailure)
            return StatusCode(
                result.StatusCode,
                ApiResponse<TableTypeDto>.ErrorResponse(
                    result.Error,
    ...
    public async Task<ActionResult<ApiResponse<TableTypeDto>>> Create(
        [FromBody] CreateTableTypeDto dto,
        CancellationToken ct = default
    )
    {
        if (!ModelState.IsValid)
            return BadRequest(
                ApiResponse<TableTypeDto>.ErrorResponse("Invalid model", ErrorCodes.ValidationError)
            );
    ...
    )
    {
        if (id != dto.Id)
            return BadRequest(
                ApiResponse<TableTypeDto>.ErrorResponse("ID mismatch", ErrorCodes.ValidationError)
            );
        ...
        return Ok(
            ApiResponse<TableTypeDto>.SuccessResponse(updatedResult.Value, "Table type updated")
        );
    }
