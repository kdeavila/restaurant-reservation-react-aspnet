...
public class UsersController(IUserService userService) : ControllerBase
{
   ...
   [HttpGet]
   public async Task<ActionResult<ApiResponse<IEnumerable<UserDto>>>>
   GetAll([FromQuery] UserQueryParams queryParams, CancellationToken ct = default)
   {
   ...
   public async Task<ActionResult<ApiResponse<UserDto>>> GetById(string id, CancellationToken ct = default)
   {
      ...
      if (result.IsFailure)
         return StatusCode(result.StatusCode, ApiResponse<UserDto>.ErrorResponse(result.Error,
         GetErrorCode(result.StatusCode), result.StatusCode));
   ...
   public async Task<ActionResult<ApiResponse<string>>> Deactivate(string id, CancellationToken ct = default)
   {
      ...
      if (result.IsFailure)
         return StatusCode(result.StatusCode, ApiResponse<string>.ErrorResponse(result.Error,
         GetErrorCode(result.StatusCode), result.StatusCode));
