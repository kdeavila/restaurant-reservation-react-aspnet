...
public class ClientsController(IClientService clientService) : ControllerBase
{
    ...
    )
    {
        ...
        return Ok(
            ApiResponse<IEnumerable<ClientDto>>.SuccessResponse(
                data,
    ...
    )
    {
        ...
        if (result.IsFailure)
            return StatusCode(
                result.StatusCode,
                ApiResponse<ClientDto>.ErrorResponse(
                    result.Error,
    ...
    )
    {
        if (!ModelState.IsValid)
            return BadRequest(
                ApiResponse<ClientDto>.ErrorResponse("Invalid model", ErrorCodes.ValidationError)
            );
    ...
    )
    {
        if (id != dto.Id)
            return BadRequest(
                ApiResponse<ClientDto>.ErrorResponse("ID mismatch", ErrorCodes.ValidationError)
            );
