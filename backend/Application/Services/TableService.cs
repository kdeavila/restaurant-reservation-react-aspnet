...
) : ITableService
{
  ...
  public async Task<IEnumerable<TableDetailedDto>> GetAvailableTablesAsync(
  ...
  )
  {
    ...
    return availableTables.Select(table => table.Adapt<TableDetailedDto>());
  }
  ...
  public async Task<Result> UpdateAsync(UpdateTableDto dto, CancellationToken ct = default)
  {
    ...
    {
      // Validate if trying to set status to Inactive
      if (parsed == TableStatus.Inactive && table.Status != TableStatus.Inactive)
      {
        var tableWithReservations = await _tableRepository.GetByIdWithReservationsAsync(dto.Id, ct);
        if (tableWithReservations != null)
        {
          var activeReservations = tableWithReservations.Reservations.Where(r =>
          ...
          if (activeReservations.Any())
            return Result.Failure(
                $"Cannot deactivate table. It has {activeReservations.Count} active reservation(s).",
                409
            );
        }
      }
  ...
  {
    var table = await _tableRepository.GetByIdWithReservationsAsync(id, ct);
    if (table is null)
