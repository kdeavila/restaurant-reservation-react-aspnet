...
public class TableRepository(RestaurantReservationDbContext context) : ITableRepository
{
    ...
    public IQueryable<Table> Query() => _context.Tables.Include(t => t.TableType).AsNoTracking();
    ...
    public async Task<IEnumerable<Table>> GetByTableTypeIdAsync(
        ...
        await _context
            .Tables.Where(t => t.TableTypeId == tableTypeId)
            .AsNoTracking()
            .ToListAsync(ct);

    public async Task<Dictionary<int, int>> GetTableCountsByTableTypeIdsAsync(
        IEnumerable<int> tableTypeIds,
        CancellationToken ct = default
    ) =>
        await _context
            .Tables.Where(t => tableTypeIds.Contains(t.TableTypeId))
