...
public class ReservationRepository(RestaurantReservationDbContext context) : IReservationRepository
{
    ...
    public async Task<IEnumerable<Reservation>> GetByClientIdAsync(
        int clientId,
        CancellationToken ct = default
    ) =>
        await _context
            .Reservations.Where(r => r.ClientId == clientId)
            .Include(r => r.Table)
            .AsNoTracking()
            .ToListAsync(ct);

    public async Task<IEnumerable<Reservation>> GetByTableIdAsync(
        int tableId,
        CancellationToken ct = default
    ) =>
        await _context
            .Reservations.Where(r => r.TableId == tableId)
            .Include(r => r.Client)
            .AsNoTracking()
            .ToListAsync(ct);
